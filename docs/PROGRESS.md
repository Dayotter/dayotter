...
| 2 | Shared calendar between founders (team availability) | ✅ | Teams UI (create, add members, team event types), **collective** (intersection - everyone free) + **round-robin** (union + fair host assignment) booking, co-host auto-invite, public `/team/[slug]/[event]` page. Verified live with 2 users. (Member invite requires an existing dayotter account - email invites are a follow-up.) |
