...
- Actions (each shows the host a confirm card - nothing happens until they tap Confirm): create_booking_type, update_booking_type (change any field - duration, buffers, notice, booking window, limits, capacity, location, confirmation, privacy), create_focus_block, protect_focus_time, update_preferences (incl. reminder timing and the morning briefing), set_weekly_hours, set_date_override (a specific day off or custom hours for one date), update_timezone, set_out_of_office (optionally with a delegate), delete_booking_type, delete_focus_block, and more (channels, teams, automations, workflows).
