 ...
 */
export async function fetchIcsFeed(rawUrl: string, redirectsLeft = MAX_REDIRECTS): Promise<string> {
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        if (status >= 300 && status < 400 && res.headers.location) {
          ...
          fetchIcsFeed(next, redirectsLeft - 1).then(resolve, reject);
          return;
        }
        if (status !== 200) {
          res.resume();
...
/**
 * Parse an ICS document into full {@link SyncedEvent}s overlapping [from, to].
 * Recurrence-aware (expands RRULE occurrences, honours EXDATE and moved
...
export function parseIcsEvents(icsText: string, from: Date, to: Date): SyncedEvent[] {
  ...
  for (const key of Object.keys(data)) {
    const ev = data[key] as RawVEvent;
    ...
    const transparency = ev.transparency === "TRANSPARENT" ? "transparent" : "opaque";
    ...
    if (ev.rrule) {
      ...
      for (const raw of ev.rrule.between(from, to, true)) {
        ...
        const oEnd = new Date(oStart.getTime() + durationMs);
        out.push({
          ...base,
          externalEventId: `${uid}#${oStart.toISOString()}`,
          start: oStart,
          end: oEnd,
          isRecurring: true,
      ...
      for (const r of Object.values(ev.recurrences ?? {})) {
        ...
        if (rs && re && overlaps(rs, re)) {
          out.push({
            ...base,
            externalEventId: `${uid}#${rs.toISOString()}`,
            start: rs,
            end: re,
            isRecurring: true,
            recurringEventId: uid,
      ...
      out.push({ ...base, externalEventId: uid, start, end });
    }
  }
...
/** Derive opaque busy intervals from parsed events (drops free/transparent). */
export function eventsToBusy(events: SyncedEvent[]): BusyInterval[] {
  return events
